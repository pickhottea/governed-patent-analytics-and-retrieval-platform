...
# -------- component extraction (keep your conservative approach) --------
...
BAD_SNIPPETS = [
    ...
    # NEW: kill legal boilerplate defs
...
TYPE_PRIORITY = {"spec_def": 0, "spec_summary": 1, "spec_embodiment": 2}
...
def contains_component(txt: str, comp: str) -> bool:
    ...
    if not c:
        return False
    # strict contains first (simple + robust)
    if c in t:
        return True
    # weak fallback: head noun (last token) appears
...
def rerank(hits: List[Dict[str, Any]], topk: int, comp: str) -> List[Dict[str, Any]]:
    # 1) drop boilerplate / junk
    hits1 = [h for h in hits if not is_bad_hit_text(h["_full"])]

    # 2) prefer hits that actually mention the component
    mention = [h for h in hits1 if contains_component(h["_full"], comp)]
    cand = mention if len(mention) >= 2 else hits1

    # 3) prefer non-topic-noise if possible
    non_noise = [h for h in cand if not is_topic_noise(h["_full"])]
    cand2 = non_noise if len(non_noise) >= 2 else cand

    # 4) chunk_type priority then distance
    def key(h):
...
def main() -> int:
    ...
    ap.add_argument("--out", default="artifacts/map_claim_to_def_v5_family_first_rerank.jsonl")
    ap.add_argument("--only_independent", action="store_true", default=True)
    ap.add_argument("--family_n_results", type=int, default=80)
    ap.add_argument("--global_n_results", type=int, default=80)
    args = ap.parse_args()
